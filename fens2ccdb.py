"""
   Script to bulk-evaluate FENs with Chinese chessdb.cn.
"""
import argparse, asyncio, sys, time, ccdblib
...
class fens2ccdb:
    ...
    async def parse_single_line(self, line):
        if line.startswith("#"):  # ignore comments
            return line
        fen = " ".join(line.split()[:4])  # ccdb ignores move counters anyway
        ...
        if self.shortFormat:
            ...
            score = f"ccdb eval: {score}"
        return f"{line}{';' if line[-1] != ';' else ''} {score};"


async def main():
    parser = argparse.ArgumentParser(
        description='A simple script to request evals from Chinese chessdb.cn for a list of FENs stored in a file. The script will add "; EVALSTRING;" to every line containing a FEN. Lines beginning with "#" are ignored, as well as any text after the first four fields of each FEN.',
