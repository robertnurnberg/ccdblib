"""
   Script that makes Chinese chessdb.cn explore certain openings or book exits.
"""
import argparse, asyncio, math, random, time, sys, os, ccdblib
...
class ccdbwalk:
    ...
    async def parse_single_line(self, lineIdx):
        ...
        if self.isPGN:
            ...
            board = cchess.Board(line)
        r = await self.ccdb.showall(board.epd())
        ...
        while "moves" in r and ply - ply0 < self.depthLimit:
            ...
            if o := board.outcome() is not None:
                ...
                if self.verbose:
                    retStr += o.result()
            elif not self.TBwalk and (
                (pc := sum(p in "rnckabp" for p in board.epd().lower().split()[0])) <= 9
                or (apc := sum(p in "rnc" for p in board.epd().lower().split()[0])) <= 3
            ):
                r = {}
                if self.verbose:
                    retStr += f"[{pc}pieces w/ {apc}attackers limit]"
            else:
                r = await self.ccdb.queryall(board.epd())
        if self.verbose >= 3:
            retStr += f'\n  URL: {url.replace(" ", "_")}'
        bt = 0
        while bt <= self.backtrack:
            asyncio.ensure_future(self.ccdb.queue(board.epd()))
            bt += 1
...
async def main():
    parser = argparse.ArgumentParser(
        description="A script that walks within the Chinese chessdb.cn tree, starting from FENs or lines in a PGN file. Based on the given parameters, the script selects a move in each node, walking towards the leafs. Once an unknown position is reached, it is queued for analysis and the walk terminates.",
